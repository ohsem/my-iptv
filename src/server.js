...
const unifiTVVODTemplateURL = 'https://raw.githubusercontent.com/weareblahs/unifi-tv/main/dev/vod.ejs'
...
module.exports = (fastify, options, next) => {
  ...
  const unifiTVm3u8VODTemplate = async () => {
    if (nodeCache.get("unifiTVVODTemplate")) return nodeCache.get("unifiTVVODTemplate");
    else {
      const { data } = await axios.get(unifiTVVODTemplateURL)
      nodeCache.set("unifiTVVODTemplate", data, DEFAULT_TEMPLATE_TTL)
      return data
    }
  }
  ...
  fastify.get('/vod/unifi-tv', async ({ query }, res) => {
    const { userID, clientPasswd } = query
    const userKey = cacheKey(`${userID}:${clientPasswd}`)
    const ejsTemplate = await unifiTVm3u8VODTemplate()
